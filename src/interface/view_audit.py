...
# ⚡ PREVENT SILENT CRASHES IF PROFILES FAIL TO LOAD
try:
    from src.utils.date_profiles import DATE_PROFILES
except ImportError:
    print("⚠️ WARNING: DATE_PROFILES could not be imported in view_audit.")
        ...
        dbc.Row([
            dbc.Col([
                ...
                dbc.Select(
                    ...
                    value='BACKTEST', 
                    style=INPUT_STYLE, className="mb-3 text-dark"
                )
            ], width=6),
            dbc.Col([
...
# ==============================================================================
@callback(
    [Output('audit-chart-hourly', 'figure'), Output('audit-chart-daily', 'figure'),
     Output('audit-chart-duration', 'figure'), Output('audit-table-container', 'children')],
    [Input('audit-source', 'value'), Input('audit-profile', 'value')]
)
def update_audit(source, profile_name):
    if not source: return no_update, no_update, no_update, no_update
    empty_fig = go.Figure().update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')

    try:
        con = duckdb.connect(str(config.DB_FILE), read_only=True)
        
        # ⚡ DATE PARSING FAILSAFE (Handles Strings or Objects)
        ...
        if source == 'rh':
            q = f"SELECT entry_time_utc as entry_time, entry_time_utc as exit_time, opra_code as ticker, action, fill_price as entry_price, 0.0 as exit_price, net_pnl as pnl FROM active_rh_log WHERE entry_time_utc >= '{start_str}'"
        else:
            q = f"SELECT entry_time, exit_time, ticker, action, entry_price, exit_price, net_pnl as pnl FROM {config.TBL_SIM_LOG} WHERE source_id = '{source}' AND entry_time >= '{start_str}'"
        ...
        fig_hourly = px.bar(hourly_pnl, x='hour', y='pnl', color='pnl', color_continuous_scale=['#ef4444', '#334155', '#22c55e'])
        fig_hourly.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family="monospace"), showlegend=False, coloraxis_showscale=False)
        fig_hourly.add_vrect(x0=6.5, x1=7.5, annotation_text="OPEN", annotation_position="top left", fillcolor="yellow", opacity=0.1, line_width=0)

        daily_pnl = df.groupby('weekday', observed=False)['pnl'].sum().reset_index()
        fig_daily = px.bar(daily_pnl, x='weekday', y='pnl', color='pnl', color_continuous_scale=['#ef4444', '#334155', '#22c55e'])
        fig_daily.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family="monospace"), showlegend=False, coloraxis_showscale=False)

        df['Outcome'] = df['pnl'].apply(lambda x: 'WIN' if x > 0 else 'LOSS')
        fig_dur = px.scatter(df, x='duration_mins', y='pnl', color='Outcome', color_discrete_map={'WIN': '#22c55e', 'LOSS': '#ef4444'}, hover_data=['ticker', 'entry_time'])
        fig_dur.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family="monospace"))
        ...
        tbl = dash_table.DataTable(
            ...
            style_header={'backgroundColor': '#1e293b', 'color': '#f8fafc', 'fontWeight': 'bold', 'borderBottom': '2px solid #475569', 'fontFamily': 'monospace'},
            style_data={'backgroundColor': '#0f172a', 'color': '#f8fafc', 'border': '1px solid #334155', 'fontFamily': 'monospace', 'fontSize': '13px'},
            style_cell={'textAlign': 'left', 'padding': '10px'},
            # ⚡ REACT CRASH FIX: Safe, layered query syntax
            style_data_conditional=[
                {'if': {'column_id': 'pnl_str'}, 'color': '#10b981', 'fontWeight': 'bold'}, 
                {'if': {'filter_query': '{pnl_str} contains "-"', 'column_id': 'pnl_str'}, 'color': '#ef4444', 'fontWeight': 'bold'} 
            ],
            page_size=15
        )
        ...
        return empty_fig, empty_fig, empty_fig, html.Div(f"ERROR EXTRACTING VAULT DATA: {e}", className="text-danger fw-bold font-monospace")