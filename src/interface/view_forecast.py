...
# 1. PATH CONSTITUTION
# ==============================================================================
# File: src/interface/view_forecast.py
# Root: ../../
...
def calculate_silver_arrow(start_bal, daily_goal_pct, business_days):
    """
    Calculates the exponential growth curve based on the 'Silver Arrow' daily target.
    """
...
def fetch_simulation_seed_from_db():
    """Queries the 'active_simulation_log' table from the Vault to seed Monte Carlo."""
...
layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            html.H2("CAPITAL FORECASTER", className="display-6 fw-bold text-white"),
            html.Small("MONTE CARLO SIMULATION vs SILVER ARROW TARGET", className="text-muted"),
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardHeader("PROBABILITY CONE vs. TARGET", className="fw-bold text-primary"),
                dbc.CardBody([
...
def update_forecast(n_clicks, start_cap, daily_goal, start_date, end_date, risk_pct):
    ...
    # Draw 50 random paths for visual texture
