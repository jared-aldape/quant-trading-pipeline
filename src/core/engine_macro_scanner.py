...
def get_fundamental_bias():
    ...
    Reads from 'data/macro_sentiment.json' generated by Step 0.
    """
    path = ROOT_DIR / "data" / "macro_sentiment.json"
    if not path.exists():
        return {"bias": "NEUTRAL", "active": False}
...
def apply_macro_logic(df):
    ...
    """
    # PATCH: Use standard Pandas instead of pandas_ta to avoid DataFrame return errors
    df['vix_sma'] = df['close'].rolling(window=20).mean()
    ...
    for i in range(1, len(df)):
        ...
        # Skip if SMA hasn't calculated yet (need 20 days of data)
            ...
            if fund_data['bias'] == "BULLISH":
                if vix_cross_under:
                    ...
                    signal = {
                        ...
                        'trade_type': 'call',
                        'reason': f"VIX Tech Buy + {fund_data.get('event', 'Macro')} (Strong)"
                    }
                    ...
                    signal = {
                        ...
                        'trade_type': 'call',
                        'reason': f"Regime Continuation ({fund_data.get('event', 'Macro')})"
                    }
            ...
            # B. BEARISH REGIME (e.g., Tech Sell-off)
                ...
                if vix_cross_over:
                    ...
                    signal = {
                        ...
                        'trade_type': 'put',
                        'reason': f"VIX Tech Sell + {fund_data.get('event', 'Macro')} (Panic)"
                    }
...
def run_macro_scan():
    ...
    try:
        ...
        if signal_df.empty:
            log.warning("⚠️ No Macro Signals generated (Need more history for SMA).")
            con.close()
            return
