...
def black_scholes(S, K, T, r, sigma, option_type="call"):
    ...
    CRITICAL FIX: Enforces Intrinsic Value (0 extrinsic) when T <= 0.
    """
...
def get_time_to_close():
    """
    Calculates fraction of year remaining until 16:00 ET today.
    CRITICAL FIX: Returns exactly 0.0 if market is closed/past 4pm.
    """
    now = datetime.now(config.TZ_NY)
    market_close = now.replace(hour=16, minute=0, second=0, microsecond=0)
    
    if now >= market_close: 
        return 0.0 # <--- HARD DECK: Expired means Expired.
...
def load_session():
    ...
    try:
        with open(SESSION_FILE, 'r') as f: 
            data = json.load(f)
            if 'start_balance' not in data: data['start_balance'] = 600.0
...
def get_portfolio_stats():
    """Calculates Total Account Value (Liquid + Open Positions M2M)"""
    state = load_session()
...
def execute_entry(trade_type, size_val, size_mode="AMT", fee_model="RH_GOLD"):
    ...
    else: num_contracts = int(size_val / (contract_cost + 0.40))
